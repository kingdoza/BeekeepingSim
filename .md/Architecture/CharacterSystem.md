...
- `ABeekeeperController`: Enhanced Input mapping 등록, active storage/drag operation 저장
...
2. `ABeekeeperCharacter`가 이동, 시점, 점프, 스프린트, focus, hotbar 입력을 하위 컴포넌트로 라우팅한다.
3. Focus engaged 중에는 `SetFocusInteractionInputLocked()`로 일반 입력을 제한한다.
4. `UBeekeeperHeldItemVisualizerComponent`는 hotbar/focus delegate를 구독하고, 로컬 플레이어에서만 presentation actor를 유지한다.
5. Storage UI가 열리면 controller가 active storage와 active drag operation을 저장해 UI slot resolution의 기준점이 된다.
