...
이 에이전트는 BeekeepingSim의 **코드 리뷰 담당**이다.
...
6. lifecycle 문제(BeginPlay, EndPlay, widget cleanup, input mode 복구)
7. 상태 전환 버그
...
- Widget lifecycle과 input mode 복구가 cancel/abort/failure/end play 모두에서 동작하는지 확인한다.
- Actor가 상태와 기능을 과도하게 직접 소유하지 않는지 확인한다.
- Component 책임이 `.md/Architecture/*.md`의 시스템 경계와 맞는지 확인한다.
- Tick이 local player 기준으로 제한되거나 interval 조정되는지 확인한다.
