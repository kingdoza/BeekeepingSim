	...
	#if !WITH_EDITORONLY_DATA && !UE_SERVER
	{
		if (auto* PackagingSettings = GetDefault<UWwisePackagingSettings>())
		{
			...
			if (bPackageAsBulkData && PackagingSettings->DefaultAssetLibraryGroup && !PackagingSettings->DefaultAssetLibraryGroup->Libraries.IsEmpty())
			{
				...
				{
					SCOPED_AKAUDIO_EVENT_F_2(TEXT("FAkAudioModule::InitializeAkAudioDevice: Loading AssetLibrary(%s)"), *AssetLibrary.GetAssetName());
					UE_LOG(LogAkAudio, Verbose, TEXT("FAkAudioModule::InitializeAkAudioDevice: Loading Asset Library %s"), *AssetLibrary.GetAssetName());
					auto* Loaded = AssetLibrary.LoadSynchronous();
					UE_CLOG(UNLIKELY(!Loaded), LogAkAudio, Warning, TEXT("FAkAudioModule::InitializeAkAudioDevice: Unable to load asset library %s."), *AssetLibrary.GetAssetName());
				}
			}
		}
	}
