...
2. 프로젝트 디렉토리 구조를 파악한다
3. 작업과 관련된 핵심 파일을 우선적으로 탐색한다
4. 필요 시 범위를 점진적으로 확장하여 추가 파일을 분석한다
5. 기존 클래스 구조, 변수명, 함수명, 코딩 컨벤션을 파악한다
6. 파악한 내용을 요약해서 보고한다
7. 지시를 기다린다
...
- 명시적인 코드 작성 지시가 없을 때
- 요구사항이 불명확할 때 (명확해질 때까지 질문한다)
- 기존 코드와 충돌 가능성이 있을 때 (충돌 여부를 먼저 보고한다)
